...
/**
 * Main login page. (Modified to make auto-login work with the CCRS application in-app browser)
 *
 * @package    core
 * @subpackage auth
...
//VCH 2016/28/06 - this parameter is received from the intial post and then again from the testsession pary of Moodles default script
$vch_courseid	= optional_param('vchcid', 0, PARAM_INT); //doesnt work
$SESSION->ccrs_keyneeded = false;
...
if ($testsession) {
    if ($testsession == $USER->id) {
        if (isset($SESSION->wantsurl)) {
			//VCH 2016/28/06 - Send the users on their way
            $SESSION->wantsurl   = $CFG->wwwroot .'/course/view.php?id='.$vch_courseid.''; 
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    /// Currently supported only for ldap-authentication module
        ...
        //VCH 2016/04/07 - added the courseid to urltogo
        $SESSION->wantsurl   = $CFG->wwwroot .'/course/view.php?id='.$vch_courseid.''; 
        $urltogo = $SESSION->wantsurl;

        //VCH 2016/28/06 - auto enrol users    
        if (isloggedin()) {
            $enrols = enrol_get_plugins(true);
            $enrolinstances = enrol_get_instances($vch_courseid, true);
            foreach($enrolinstances as $instance) {
                if (!isset($enrols[$instance->enrol])) {
                    continue;
                }
                if($instance->enrol=="self" && isset($SESSION->ccrs_keyneeded) && $SESSION->ccrs_keyneeded === false) {
                    unset($SESSION->ccrs_keyneeded);
                    $enrols["self"]->enrol_user($instance, $USER->id,5);
                }
            }
        }

		//VCH 2016/28/06 - added the courseid to the querystring
        redirect(new moodle_url(get_login_url(), array('testsession'=>$USER->id,'vchcid'=>$vch_courseid)));
...
//VCH
//$SESSION->wantsurl = $CFG->wwwroot .'/course/view.php?id='.$vch_courseid; //doesnt work
